...
class RoomServiceTest {
    ...
    @Test
    void createRoom_usesResolvedHostNameAndClampsPlayerCount() {
        CreateRoomRequest request = new CreateRoomRequest();
        request.setHostName("Quiz Master");
        ...
        assertThat(room.getParticipants())
            ...
            .containsExactly("host-player", "Quiz Master");
    ...
    @Test
    void joinRoom_rejectsStartedRoom() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        room.setStatus("playing");

        RoomActionResult<RoomState> result = service.joinRoom("guest-client", joinRequest(room.getRoomCode(), "Guest"));

        assertThat(result.isOk()).isFalse();
        assertThat(result.getError()).isEqualTo("Game already started. Cannot join now.");
    }
    ...
    void joinRoom_returnsExistingRoomWhenClientAlreadyJoined() {
        ...
        RoomActionResult<RoomState> result = service.joinRoom("guest-client", joinRequest(room.getRoomCode(), "Replacement Name"));
    ...
    @Test
    void joinRoom_rejectsMissingRoomAndUsesDefaultGuestName() {
        RoomActionResult<RoomState> missing = service.joinRoom("guest-client", joinRequest("missing", "Guest"));

        assertThat(missing.isOk()).isFalse();
        assertThat(missing.getError()).isEqualTo("Room not found. Check the code and try again.");
    ...
    void joinRoom_rejectsFullRoom() {
        ...
        RoomActionResult<RoomState> result = service.joinRoom("late-client", joinRequest(room.getRoomCode(), "Late"));
    ...
    @Test
    void publishState_marksRoomFinishedForResultsPhase() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        room.setPendingAnswer(new RoomState.PendingAnswer());
        room.setSystemMessage("stale");

        RoomState.BroadcastGameState gameState = new RoomState.BroadcastGameState();
        gameState.setPhase("results");
        RoomState.GameSettings settings = new RoomState.GameSettings();
        settings.setCategory("science");
        ...
        assertThat(updated.getSettings()).isSameAs(settings);
        assertThat(updated.getPendingAnswer()).isNull();
        assertThat(updated.getSystemMessage()).isNull();
    }

    @Test
    void publishState_marksRoomPlayingWhenPhaseIsNotResultsAndPreservesSettingsWhenMissing() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        RoomState.GameSettings existingSettings = new RoomState.GameSettings();
        existingSettings.setDifficulty("hard");
    ...
    @Test
    void submitAnswer_recordsPendingAnswerForGuest() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        service.joinRoom("guest-client", joinRequest(room.getRoomCode(), "Guest"));
        room.setStatus("playing");
        ...
        payload.setRoomCode(room.getRoomCode());
        payload.setAnswer("Paris");
        payload.setTimedOut(true);

        RoomState updated = service.submitAnswer("guest-client", payload);

        assertThat(updated).isSameAs(room);
        assertThat(updated.getPendingAnswer()).isNotNull();
        assertThat(updated.getPendingAnswer().getSubmissionId()).isEqualTo(1L);
        assertThat(updated.getPendingAnswer().getPlayerId()).isEqualTo("guest-player-2");
        assertThat(updated.getPendingAnswer().getAnswer()).isEqualTo("Paris");
        assertThat(updated.getPendingAnswer().isTimedOut()).isTrue();
    }

    @Test
    void submitAnswer_rejectsHostAndIncrementsSequenceAcrossSubmissions() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        service.joinRoom("guest-client", joinRequest(room.getRoomCode(), "Guest"));
        room.setStatus("playing");

        SubmitAnswerPayload hostPayload = new SubmitAnswerPayload();
        hostPayload.setRoomCode(room.getRoomCode());
        hostPayload.setAnswer("Host guess");

        assertThat(service.submitAnswer("host-client", hostPayload)).isNull();

        SubmitAnswerPayload guestPayload = new SubmitAnswerPayload();
        guestPayload.setRoomCode(room.getRoomCode());
        guestPayload.setAnswer("First");
        RoomState first = service.submitAnswer("guest-client", guestPayload);

        guestPayload.setAnswer("Second");
        RoomState second = service.submitAnswer("guest-client", guestPayload);

        assertThat(first).isSameAs(room);
        assertThat(second).isSameAs(room);
        assertThat(room.getPendingAnswer().getSubmissionId()).isEqualTo(2L);
        assertThat(room.getPendingAnswer().getAnswer()).isEqualTo("Second");
    }

    @Test
    void submitAnswer_returnsNullForMissingInputsUnknownRoomWrongStateOrUnknownPlayer() {
        assertThat(service.submitAnswer("guest-client", null)).isNull();

        SubmitAnswerPayload unknownRoom = new SubmitAnswerPayload();
        unknownRoom.setRoomCode("ROOM1");
        unknownRoom.setAnswer("Answer");
        assertThat(service.submitAnswer("guest-client", unknownRoom)).isNull();
        ...
        assertThat(service.submitAnswer("guest-client", payload)).isNull();

        room.setStatus("playing");
        assertThat(service.submitAnswer("missing-client", payload)).isNull();
    }
    ...
    @Test
    void disconnect_finishesActiveGameWhenTooFewPlayersRemain() {
        RoomState room = service.createRoom("host-client", new CreateRoomRequest());
        service.joinRoom("guest-client", joinRequest(room.getRoomCode(), "Guest"));
        room.setStatus("playing");
        room.setPendingAnswer(new RoomState.PendingAnswer());
