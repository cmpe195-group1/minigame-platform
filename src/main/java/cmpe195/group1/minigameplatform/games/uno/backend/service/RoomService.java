package cmpe195.group1.minigameplatform.games.uno.backend.service;
...
public class RoomService implements SnapshotRoomService<RoomState> {
    ...
    public RoomState publishState(String clientId, PublishStatePayload payload) {
        ...
        synchronized (room) {
            ...
            if ("finished".equals(gameState.getPhase())) {
    ...
    public RoomState submitAction(String clientId, SubmitActionPayload payload) {
        if (payload == null || payload.getKind() == null || payload.getKind().isBlank()) {
            return null;
        }
        ...
        synchronized (room) {
            ...
            long actionId = room.getActionSequence() + 1;
            room.setActionSequence(actionId);
            ...
            pendingAction.setKind(payload.getKind());
            pendingAction.setCardId(payload.getCardId());
            pendingAction.setChosenColor(payload.getChosenColor());
