...
export interface UseWebSocketTransportReturn {
  ...
  leaveRoom: () => void
  updateSettings: (settings: RoundSettings) => void
...
export function useWebSocketTransport(): UseWebSocketTransportReturn {
  ...
  const subscribeToRoom = useCallback((roomCode: string) => {
    ...
    const destination = `/topic/anagrams/room/${roomCode}`
  ...
  useEffect(() => {
    ...
    client.onConnect = () => {
      ...
      clientSubscriptionRef.current = client.subscribe(`/topic/anagrams/client/${clientToken}`, (message) => {
        const parsed = parseMessage(message)
        ...
        if (parsed.type === "JOIN_ERROR" || parsed.type === "ERROR") {
          setJoinError(parsed.error ?? "Unable to join anagrams room.")
    ...
    client.onDisconnect = () => {
      ...
      if (!roomStateRef.current) {
        setJoinError("Lost connection to the Anagrams server. Check that the Spring backend is still running.")
      }
    }

    client.onStompError = (frame) => {
      setJoinError(frame.headers["message"] ?? "Anagrams server rejected the connection.")
    }

    client.onWebSocketError = () => {
      setJoinError(`Unable to connect to the Anagrams server at ${resolveBrokerUrl()}`)
    }
  ...
  const publishJson = useCallback((destination: string, body: unknown) => {
    ...
    if (!client) {
      setJoinError("Anagrams connection has not been initialized yet.")
      return
    }
    ...
    setJoinError("Still connecting to the Anagrams server. Try again in a moment.")
  }, [])
  ...
  const createRoom = useCallback(
    (hostName: string, maxPlayers: number) => {
      setJoinError(null)
      setRole("host")
      publishJson("/app/anagrams/create", { hostName, maxPlayers, clientToken })
    },
  ...
  const leaveRoom = useCallback(() => {
    ...
    if (current?.roomCode) {
      publishJson("/app/anagrams/leave", { roomCode: current.roomCode })
    }
  ...
  const updateSettings = useCallback(
    (settings: RoundSettings) => {
      ...
      publishJson("/app/anagrams/settings", {
  ...
  const publishState = useCallback(
    (gameState: BroadcastGameState) => {
      ...
      publishJson("/app/anagrams/state", {
        roomCode: current.roomCode,
        gameState,
      })
    },
  ...
  const submitWord = useCallback(
    (word: string) => {
      ...
      publishJson("/app/anagrams/submitWord", {
        roomCode: current.roomCode,
        word,
      })
    },
  ...
  const endTurn = useCallback(() => {
    ...
    publishJson("/app/anagrams/endTurn", {
      roomCode: current.roomCode,
    })
  }, [publishJson])
