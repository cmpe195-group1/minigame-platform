...
export function useWebSocketTransport(): UseWebSocketTransportReturn {
  ...
  const subscribeToRoom = useCallback((roomCode: string) => {
	...
	const destination = `/topic/trivia/room/${roomCode}`
  ...
  useEffect(() => {
	...
	client.onConnect = () => {
	  ...
	  clientSubscriptionRef.current = client.subscribe(`/topic/trivia/client/${clientToken}`, (message) => {
		const parsed = parseMessage(message)
		...
		if (parsed.type === "JOIN_ERROR" || parsed.type === "ERROR") {
		  setJoinError(parsed.error ?? "Unable to join trivia room.")
	...
	client.onDisconnect = () => {
	  ...
	  if (!roomStateRef.current) {
		setJoinError("Lost connection to the Trivia server. Check that the Spring backend is still running.")
	  }
	}

	client.onStompError = (frame) => {
	  setJoinError(frame.headers["message"] ?? "Trivia server rejected the connection.")
	}

	client.onWebSocketError = () => {
	  setJoinError(`Unable to connect to the Trivia server at ${resolveBrokerUrl()}`)
	}
  ...
  const publishJson = useCallback((destination: string, body: unknown) => {
	...
	if (!client) {
	  setJoinError("Trivia connection has not been initialized yet.")
	  return
	}
	...
	setJoinError("Still connecting to the Trivia server. Try again in a moment.")
  }, [])
  ...
  const createRoom = useCallback(
	(hostName: string, maxPlayers: number) => {
	  setJoinError(null)
	  setRole("host")
	  publishJson("/app/trivia/create", { hostName, maxPlayers, clientToken })
	},
  ...
  const leaveRoom = useCallback(() => {
	...
	if (current?.roomCode) {
	  publishJson("/app/trivia/leave", { roomCode: current.roomCode })
	}
  ...
  const updateSettings = useCallback(
	(settings: GameSettings) => {
	  ...
	  publishJson("/app/trivia/settings", {
  ...
  const submitAnswer = useCallback(
	(answer: string | null, timedOut: boolean) => {
	  ...
	  publishJson("/app/trivia/submitAnswer", {
		roomCode: current.roomCode,
		answer,
		timedOut,
	  })
