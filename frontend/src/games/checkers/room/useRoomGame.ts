...
import type { RoomState } from "./CheckersRoomTypes";
import type { MovePayload } from "../checkersScene";

export function useRoomGame() {
  const config = {
    gameKey: "checkers",
    createDestination: "/app/checkers/create",
    joinDestination: "/app/checkers/join",
    leaveDestination: "/app/checkers/leave",
    startDestination: "/app/checkers/start",
  };
  ...
  const sendMove = (payload: MovePayload) => {
    ...
    ws.sendAction("/app/checkers/move", {
  ...
  const resetGame = () => {
    ...
    ws.sendAction("/app/checkers/reset", {
      roomCode: ws.roomState.roomCode,
    });
  };
